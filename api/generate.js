export default async function handler(req, res) {
  ...
  try {
    ...
    // If bg_color is provided (e.g., 'FFFFFF'), it applies the background.
    // If omitted, it returns a transparent PNG.
    if (bg_color) {
    ...
    const response = await fetch('https://api.remove.bg/v1.0/removebg', {
      method: 'POST',
      headers: {
        'X-Api-Key': process.env.REMOVE_BG_API_KEY, // Set this in Vercel Project Settings
      },
      body: formData,
    });
